...
export default function EditTopic() {
	...
	const handleSubmit = async (e) => {
		...
		try {
			const response = await fetch(`/api/forum/topic/edit/${params.topicId}`, {
				method: "POST",
				headers: {
	...
	useEffect(() => {
		...
		const fetchTopic = async () => {
			try {
				const response = await fetch(`/api/forum/topic/${params.topicId}`, {
					method: "GET",
					headers: {
						"Content-Type": "application/json",
					},
				...
				const data = await response.json();
				const { title, equipId, articleId, message } = data.topic;
	...
	return (
		<main className="wrapperMain wrap-login">
			<Helmet>
				...
				<meta name="description" content="Edit a topic" />
			</Helmet>
			<h1>Edit a Topic</h1>
